...
class AIService:
    ...
    def _build_system_prompt(summary_text: str) -> str:
        ...
        return (
            "당신은 사용자의 시계열 데이터(주가, 지표, 측정치, 추세 등)를 완벽히 파악하고 있는 전문 AI 데이터 분석 비서입니다.\n\n"
            f"{summary_text}\n\n"
            "## 비서 핵심 지침:\n"
            "1. 사용자의 질문에 답변할 때, 주입된 데이터 요약(기간, 평균치, 최고치/최저치, 7일 이동평균 추세, 주요 메모 이슈 등)을 적극적으로 인용하여 명확하고 설득력 있게 답변하세요.\n"
            "2. 구체적인 수치(평균값, 최고치, 날짜 등)와 데이터 메모의 맥락을 결합하여 신뢰성 있는 전문 인사이트를 제공하세요.\n"
